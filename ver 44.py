...
alg.drop('Country Name', axis=1, inplace=True)
alg = alg.T.reset_index()
alg.columns = ['Date', 'Value']
...
# fit exponential growth
popt1, covar1 = opt.curve_fit(exp_growth, alg["Date"], alg["Value"], maxfev = 10000)

print("Fit parameter", popt1)
# use *popt to pass on the fit parameters
alg["pop_exp"] = exp_growth(alg["Date"], *popt1)
plt.figure()
plt.plot(alg["Date"], alg["Value"], label="data")
plt.plot(alg["Date"], alg["pop_exp"], label="fit")
plt.legend()
plt.title("First fit attempt")
plt.xlabel("year")
plt.ylabel("Value")
...
# fit exponential growth
popt1, covar1 = opt.curve_fit(exp_growth, alg["Date"],
alg["Value"], p0=[8e4, 0.06])
# much better
print("Fit parameter", popt1)
alg["pop_exp"] = exp_growth(alg["Date"], *popt1)
plt.figure()
